# Speaking R’s Language {#sec-speaking-rs-language}
...
```{r} 
#| label: speaking_r_factory1
#| echo: FALSE
#| fig-cap: A factory making bicycles.
knitr::include_graphics("factory1.png")
```

To make this concept more concrete, in @sec-navigating-rstudio we used the `seq()` function as a factory. Specifically, we wrote `seq(from = 2, to = 100, by = 2)`. The inputs (arguments) were `from`, `to`, and `by`. The output (returned result) was a set of numbers that went from 2 to 100 by 2's. Most functions, like the `seq()` function, will be a word or word part followed by parentheses. Other examples are the `sum()` function for addition and the `mean()` function to calculate the average value of a set of numbers.

```{r}
#| label: speaking_r_factory2
#| echo: FALSE
#| fig-cap: A function factory making numbers.
knitr::include_graphics("factory2.png")
```
...
But how do we know what the first, second, and third arguments to a function are? Do you remember our discussion about RStudio's help tab in @sec-the-files-pane? There, we saw the documentation for the `seq()` function.

```{r}
#| label: speaking_r_help
#| echo: FALSE
...
```{r}
#| label: speaking_r_objects
#| echo: FALSE
#| fig-cap: Creating the x object.
knitr::include_graphics("objects.png")
```
...
```{r}
#| label: speaking_r_language
#| echo: FALSE
knitr::include_graphics("language.png")
```

## Comments
 
And finally, there are comments. If our R code is a conversation we are having with the R interpreter, then comments are our inner thoughts taking place during the conversation. Comments don’t actually mean anything to R, but they will be extremely important for us. We've actually already seen a couple examples of comments in previous lessons. 
...
Most beginning R programmers underestimate the importance of comments. In the silly little examples above, the comments are not that useful. However, comments will become extremely important as we begin writing more complex programs. When working on projects, we will often need to share our programs with others. Reading R code without any context is really challenging -- even for experienced R programmers. Additionally, even if our collaborators can surmise _what_ our R code is doing, they may have no idea _why_ we are doing it. Therefore, our comments should tell others what our code does (if it isn't completely obvious), and more importantly, what our code is trying to accomplish. Even if we aren't sharing our code with others, we may need to come back and revise or reuse our code months or years down the line. You may be shocked at how foreign the code _you wrote_ will seem months or years after you wrote it. Therefore, comments are not just important for others, they are also important for future you!
...
That second advantage is probably really confusing if this is not a concept you are already familiar with. For example, when we install Microsoft Word on our computer, all the code that makes that program work is owned and Maintained by the Microsoft corporation. If we need Word to do something that it doesn’t currently do, our only option is to make a feature request on Microsoft’s website. Microsoft may or may not get around to fulfilling that request.

R works a little differently. When we downloaded R from the CRAN website, we actually downloaded something called **Base R**. Base R is maintained by the R Core Team. However, anybody – _even you_ – can write their own code (called packages) that add new functions to the R syntax. Like all functions, these new functions allow us to _do_ things that we can't do (or can't do as easily) with Base R.

In their book, [ModernDive](https://moderndive.com/1-getting-started.html#packages), Ismay and Kim use the following mobile phone analogy.
...
So, when we get a new smart phone it comes with apps for making phone calls, checking email, and sending text messages. But, what if we want to listen to music on Spotify? We may or may not be able to do that through our phone's web browser, but it's way more convenient and powerful to download and install the Spotify app.
...
In the same way that we have to download and install Spotify on our mobile phone before we can use it, we have to download and install new R packages on our computer before we can use the functions they contain. Fortunately, R makes this really easy. For most packages, all we have to do is run the `install.packages()` function in the R console. For example, here is how we would install the `dplyr` package. 

```{r}
#| eval: FALSE
# Make sure to remember to wrap the name of the package in single or double quotes.
install.packages("dplyr")
```

Over time, we will download and install a lot of different packages. All those packages with all of those new functions start to create a lot of overhead. Therefore, R doesn't keep them loaded and available for use at all times. Instead, _every time_ we open RStudio, we will have to explicitly tell R which packages we want to use. So, when we close RStudio and open it again, the only functions that we will be able to use are Base R functions. If we want to use functions from any other package (e.g., `dplyr`) we will have to tell R that we want to do so using the `library()` function. 
...
Technically, loading the package with the `library()` function is not the only way to use a function from a package we've downloaded. For example, the `dplyr` package contains a function called `filter()` that helps us keep or drop certain rows in a data frame. To use this function, we have to first download the `dplyr` package. Then we can use the `filter()` function in one of two different ways.
...
The first way we already saw above. Load all the functions contained in the `dplyr` package using the `library()` function. Then, use that function just like any other Base R function. 

The second way is something called the **double colon syntax**. To use the double colon syntax, we type the package name, two colons, and the name of the function we want to use from the package. Here is an example of the double colon syntax:
...
Most of the time we will load packages using the `library()` function. However, we wanted to show you the double colon syntax because you may come across it when you are reading R documentation, and because there are times when it makes sense to use this syntax. 

## Programming style

Finally, we want to discuss programming style. R can read any code you write as long as you write it using valid R syntax. However, R code can be much easier or harder for people (including you) to read depending on how it's written. @sec-coding-best-practices of this book gives complete details on writing R code that is as easy as possible for _people_ to read. So, please make sure to read it. It will make things so much easier for all of us!
