...
```{r echo=FALSE}
knitr::include_graphics("img/03_part_descriptive_analysis/flowchart_cat_numerical.png")
```
...
```{r echo=FALSE}
knitr::include_graphics("img/03_part_descriptive_analysis/categorical_variables_01.jpeg")
```
...
```{r echo=FALSE}
knitr::include_graphics("img/03_part_descriptive_analysis/categorical_variables_02.jpeg")
```
...
```{r echo=FALSE}
knitr::include_graphics("img/03_part_descriptive_analysis/categorical_variables_03.jpeg")
```
...
* Also notice that you can use the "Next" button at the bottom right corner of the printed data frame to view rows 11 through 20.   
...
As is always the case with R, there are multiple ways we can calculate the statistics that we're interested in -- currently frequencies. For example, we can use the base R `table` function like this:
...
The final way I'm going to discuss here is the `tidyverse` way, which is my preference. We will have to write a little additional code, but the end result will be more flexible, more readable, and will return our statistics to us in a special kind of data frame called a **tibble** that we can save and use for further analysis. Let's walk through this step by step...

<p class="note"> 🗒**Side Note:** You should already be familiar with the pipe operator (`%>%`), but if it doesn't look familiar to you, you can learn more about it [Using pipes]. Don't forget, if you are using RStudio, you can use the keyboard shortcut `shift + command + m` (Mac) or `shift + control + m` (Windows) to type the pipe operator.</p>
...
So, what we've done above is pass our entire data frame to the `summarise()` function and asked it to count the number of rows in the data frame. The result we get is a new data frame with 1 column (named `n()`) and one row with the value 20 (the number of rows in the original data frame).
...
* So, we used `dplyr`'s [mutate()](https://dplyr.tidyverse.org/reference/mutate.html) function to create a new variable in the data frame named `prop`. Again, we could have given it any valid name.   
  - We will talk more about `mutate()` in the data management part of the book. For now, just know that it is the primary function will use for adding new columns (variables) to our data frames.   
...
* We used `filter()` to keep only the rows that have a _non-missing_ value for sex.    
  ...
  - So, `filter()` tells R which rows of a data frame to _keep_ and `is.na(sex)` tells R to find rows with an NA value for the variable `sex`. Together, `filter(is.na(sex))` would tell R to _keep_ rows with an NA value for the variable `sex`. Adding the NOT operator `!` tells R to _keep_ rows that are do _NOT_ have an NA value for the  variable `sex`.    
...
* We passed the calculated percentage values `(n / sum(n) * 100)` to the `round()` function using a pipe to round our percentages to 2 decimal places.    
  
  - Notice that we had to wrap `n / sum(n) * 100` in parentheses in order to pass it the `round()` function with a pipe.    
