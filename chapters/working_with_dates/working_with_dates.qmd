...
We won't need the `today` column anymore, so we'll go ahead and drop it here.
...
Much of the data we work with in epidemiology includes dates. In fact, it isn't uncommon for the length of time that passes between to events to be the primary outcome that we are trying to understand. Hopefully, the tools we've learned in this chapter will give you a solid foundation for working with dates in R. For more information on dates, including a handy cheat sheet, we recommend visiting the [lubridate website](https://lubridate.tidyverse.org/).