...
**Incidence**, on the other hand, is our attempt to quantify how frequently _new incidents_ of a particular health-related state occur in our population of interest during a specified period of time. We may want to quantify the incidence of events that happen one time (e.g., diagnosis of Alzheimer's disease or death), or we may want to quantify the incidence of events that can happen more than once (e.g., number of hip fractures), which are also called **recurrent events**. For example, we might want to know the quantity of people in the United States who developed a new measles infection during the previous year. We can report that quantity as a count, a proportion, an odds, or a rate. In the sections that follow, we will we discuss the differences between these measures in greater detail and show you how to use R to calculate them.

The key difference between incidence and prevalence is that incidence is a measure of _new_, as opposed to _existing_, occurrences of the condition we are attempting to quantify. Said another way, incidence quantifies the _transition_ from the absence of the condition to the presence of a condition _over a specified period of time_. @Westreich2019-lg
...
For example, if we want to estimate the point prevalence of heart disease in the United States, does that mean that we need to figure out if every single person in the United States has heart disease at the exact same moment in time? If so, that hardly seems possible. What if it takes us a day to ascertain everyone’s status? What if it takes us a month? A year? We are not aware of any rule about what duration of time is short enough to be considered a “point” in time. Therefore, we don't find the distinction between point prevalence and period prevalence to generally be particularly useful in practice. What _is_ useful, actually _critical_, in practice is to make the time frame our results apply to perfectly clear whenever we interpret and report them.
...
Because no fewer than zero people in our population of interest can be living with the health-related state of interest, and no more than all of the people in our population of interest can be living with the health-related state of interest, the prevalence proportion will range between 0 and 1.

Let's simulate some more data we can use to illustrate prevalence proportions and the other measures that will follow.
...
pop_10_arrange <- arrange(pop_10, status_f_rev)

# Create the figure
pop_10_plot <- ggplot(pop_10_arrange, aes(person_f_rev, month, fill = status_f)) +
...
Now, let's say that we come along and decide to do a study with the people in our simulated population of interest. Using equation \@ref(eq:prevalence-proportion), what is the prevalence proportion of disease in month 2? 

The prevalence proportion of disease is equal to the number of people with disease divided by the number of people in the sample. In this case, that would be 2 people (person 01 and person 02) divided by 10. 

$$\frac{2}{10} = 0.2 \hspace{1.5mm} or \hspace{1.5mm} 20\%$$

Among the members of our population, the prevalence of disease in month two was 0.2. Said another way, 20% of our population had the disease in month two.

#### Calculating prevalence proportions with R

Now, let’s look at some ways we can calculate a prevalence proportion with R.

One way (of many) to calculate our prevalence proportion is to create objects matching the elements of our equation above and then assign the relevant values to those objects. An advantage of this method is that the objects are reusable later if we need them.
...
Sometime, it can be useful to include add a percent sign to the value for reporting purposes. We can add a percent sign using the `paste0()` function as shown below. Just keep in mind adding the percent sign turns our value into a _character string_. It will no longer have a numeric value to R.
...
4. The prevalence of disease at month two in our population was 0.20 (or 20%).


### Prevalence Odds

The prevalence odds are less familiar to many people and may feel more awkward to use. However, the prevalence odds are simply a function of the prevalence proportion. Specifically, the prevalence odds are equal to the prevalence proportion divided by 1 minus the prevalence proportion. Why do we calculate the prevalence odds? We generally don't calculate the prevalence odds manually as we will do below; however, sometimes the prevalence odds are conveniently returned from regression models we will learn to use later in the book. Therefore, a gentle introduction to prevalence here may come in handy for us later.
...
In our opinion, interpretation 4, while not necessarily most commonly used, it probably the easiest for most people to understand in any meaningful way. If we go back and look at our simulated population, we can see interpretation 4 visually. For every one person who had disease at month two there were four people who did not. Two people had disease at month 2 (person 01 and person 02). Therefore, we expect that 8 people did not have disease at month 2. And that's exactly what we see -- persons 3 through 10 did not have disease in month 2.
...
2. Assuming they weren't born with this disease, person 01 _did_ newly acquire it at _some point_ in the past. In other words, they were an "incident" case at one time. However, they didn't newly acquire the disease during our _specified period of time_. This is a concrete example of the importance of being explicit about the timing and duration of our period of study.
...
n_occurrences <- 5
# Create a vector containing the number of people at risk in the population of interest
...
p09 <- c(rep("Pre-follow-up", 1), rep("At Risk", 13), rep("Diseased", 0), rep("Deceased", 0))
p10 <- c(rep("Pre-follow-up", 1), rep("At Risk", 13), rep("Diseased", 0), rep("Deceased", 0))
p11 <- c(rep("Pre-follow-up", 1), rep("At Risk", 13), rep("Diseased", 0), rep("Deceased", 0))
p12 <- c(rep("Pre-follow-up", 1), rep("At Risk", 13), rep("Diseased", 0), rep("Deceased", 0))
...
# Print pop_12 to the screen
# pop_12

# This part makes the bars stack in the correct order.
pop_12_arrange <- arrange(pop_12, status_f_rev)

# Create the figure
pop_12_plot <- ggplot(pop_12_arrange, aes(person_f_rev, month, fill = status_f)) +
  geom_col(width = 1, position = position_dodge(0), color = "black") +
  # Add a line marking the start of follow up
  geom_hline(yintercept = 0, color = "blue") +
  # Add vertical lines to mark the start of each month
...
Under these conditions, we still observe that 9 people were at risk of disease at the start of follow-up (persons 02 - 10). At some point during the 12 months of follow-up, persons 02 and 03 move out and persons 11 and 12 move in. When we go to analyze our study data at the end of follow-up, we observe that 3 people were developed the disease (person 04, person 05, and person 06). _We_ know that person 02 and person 03 also developed disease because we are simulating this scenario. Additionally, _we_ know that person 11 and person 12 weren't in the population at the start of the study because we are simulating this scenario. But, if we weren't simulating this scenario, all we would know is that we started with 10 people, 9 of whom were at risk, and that 3 of them had disease at the end of 12 months of follow-up. Therefore, we would calculate the incidence proportion as 3/9 or 0.33, which underestimates the true population incidence proportion of 0.56.

As another example, let's return to our original population of 10 people. 

```{r measures_of_occurrence_pop_10_plot_incidence_rate_loss_to_follow_up, echo=FALSE}
knitr::include_graphics("img/10_part_intro_epi/03_measures_of_occurrence/pop_10_plot.png")
```

This time, let's imagine that person 06 and person 07 decide that they don't want to participate in our study anymore and stop showing up for their follow-up appointments at month 6. This change doesn't have any affect on our denominator. We still observe that 9 people were at risk of disease at the start of follow-up (persons 02 - 10). However, we no longer know the status of person 06 and person 07 at the end of the study period (month 12). So, what value are we to use for our numerator?

Well, we may be tempted to say that neither of them developed the disease because they hadn't done so at the last follow-up visit they _did_ attend (month 5). Using that logic, we would calculate our incidence proportion as 4/9 or 0.44. Counting persons 02, 03, and 04 only in our numerator. Of course, because this is a simulated population, we know that 0.44 once again underestimates the true population incidence proportion of 0.56.

Unfortunately, loss to follow-up and movement in and out of populations are incredibly common occurrences in the real world. So, what are we to do? 

One option is to quantify incidence using methods that take the length of time each member of the population is under observation into account. Collectively, these methods are typically referred to as **survival analysis** or **time-to-event** analysis. We typically prefer to use the term "time-to-event" analysis because it more clearly communicates the fact that these methods aren't only useful when our outcome of interest is death. However, we need to introduce another key concept -- **person-time** -- before diving into time-to-event methods.
...
After we determine each person's individual time spent at risk, we add them up to the population time at risk. But let's ask R to do that for us instead of doing it by hand. 

```{r}
# Calculate the population time at risk and assign it to an object called 
# pop_time_at_risk
pop_time_at_risk <- 1 + 3 + 4 + 6 + 7 + 8 + 4 + 12 + 12
# Print the value stored in pop_time_at_risk to the screen
pop_time_at_risk
```

So, these people were under observation and at risk of developing a new occurrence of disease for a combined total of 57 months. However, it might be a little confusing if we say that we observed our population for 57 months. That can easily be interpreted to mean that our study was 57 months long. In reality, it was only 12 months long. So, we typically say that we observed our population for 57 _person-months_, or that we had 57 _person-months_ of follow-up data, or something along those lines. 

Why do we say person- _months_ instead of person- _years_? Well, it is probably the most natural why to describe the amount of person-time our population was under observation because our study was less than a year long and we collected data every month. Having said that, we are not _required_ to report the person-time under observation as person-months. We can easily convert person-months to person-years by dividing the person-months by 12.
...
```{r}
# Calculate the population time at risk in our data frame
df |> 
  summarise(
    pop_time_at_risk = sum(time_at_risk)
  )
```
...
1. The incidence rate of disease among the members of our population was 5 cases per 57 person-months during 12 months of follow-up.
...
2. The incidence rate of disease among the members of our population was 0.0877 cases per person-month during 12 months of follow-up.
...
3. The incidence rate of disease among the members of our population was 8.77 cases per 100 person-months during 12 months of follow-up.
...
4. The incidence rate of disease among the members of our population was 1.05 cases per person-year during 12 months of follow-up.
...
If you need to do a little sanity check to convince yourself that 1.05 cases per person-year is correct, then think about the following. We aleady said that there were 0.0877 per person-month in our population. We also know that we followed them for 12 months. Therefore, we would expect the person-year incidence rate to be roughly 0.0877 * 12, which is:
