...
We previously learned how to manually create a data frame in RStudio with the `data.frame()` function, the `tibble()` function, or the `tribble()` function. This will get the job done, but it’s not always very practical -- particularly when we have larger data sets. 

Additionally, others will usually share data with us that is already stored in a file of some sort. For our purposes, any file containing data that is not an R data frame is referred to as raw data. In our experience, raw data is most commonly shared as CSV (comma separated values) files or as Microsoft Excel files. CSV files will end with the **.csv** file extension and Excel files end with the **.xls** or **.xlsx** file extensions. But remember, generally speaking R can only manipulate and analyze data that has been imported into R’s global environment. In this lesson, we will learn how to take data stored in several different common types of files import them into R for use.

There are many different file types that one can use to store data. In this book, we will divide those file types into two categories: [plain text files](../appendices/glossary.qmd#plain-text-files) and [binary files](../appendices/glossary.qmd#binary-files). Plain text files are simple files that we (<i>a human</i>) can directly read using only our operating system's plain text editor (i.e., Notepad on Windows or TextEdit on Mac). These files usually end with the **.txt** file extension -- one exception being the **.csv** extension. Specifically, in this chapter we will learn to import the following variations of plain text files:
...
Later, we will discuss importing binary files. For now, we can think of binary files as more complex file types that can't generally be read by humans without the use of special software. Some examples include Microsoft Excel spreadsheets, SAS data sets, and Stata data sets.
...
If you would like to follow along, we suggest that you go ahead and install and load the `readr` package now. 
...
We will start by importing data with values are separated by a single space. Not necessarily because this is the most common format we will encounter - in our experience it isn't. We'll start with space delimited files because they are about as simple as it gets, and other types of data are often considered special cases of files separated with a single space.

:::callout-note
In programming lingo, it is common to use the word **delimited** interchangeably with the word **separated.** For example, we might say "values separated by a single space" or we might say "a file with space delimited values."
:::
...
[Click here to download this file.](https://github.com/brad-cannell/r4epi/blob/master/chapters/importing_plain_text/single_delimited.txt)
...
* We can type `?read_delim` into our R console to view the help documentation for this function and follow along with the explanation below.

* The first argument to the `read_delim()` function is the `file` argument. The value passed to the file argument should be a file path that tells R where to find the data set on our computer.
...
::: callout-warning
Make sure to always include the file extension in file paths. For example, using `/single_delimited` instead of `/single_delimited.txt` above (i.e., no `.txt`) would have resulted in an error telling us that the file does not exist.
:::
...
Sometimes we will encounter plain text files that contain values separated by tab characters instead of a single space. Files like these may be called **tab separated value** or **tsv** files, or they may be called **tab-delimited** files.
...
[Click here to download this file.](https://github.com/brad-cannell/r4epi/blob/master/chapters/importing_plain_text/tab.txt)
...
Yet another type of plain text file we will discuss is called a **fixed width format** or **fwf** file. Again, we don't encounter these files frequently, but they can be sort of tricky when we do encounter them. Take a look at this example:
...
This number line creates a sequence of columns across our data, with each column being 1 character wide. Notice that spaces are also considered a character with width just like any other. We can use these columns to tell R exactly which columns contain the values for each variable. 

[Click here to download this file.](https://github.com/brad-cannell/r4epi/blob/master/chapters/importing_plain_text/fixed_width.txt)
...
* We can type `?read_table` into our R console to view the help documentation for this function and follow along with the explanation below.

* By default, the `read_table()` function looks for values to be separated by one or more columns of space.

However, how could we import this data if there weren't always spaces in between data values? For example:
...
[Click here to download this file.](https://github.com/brad-cannell/r4epi/blob/master/chapters/importing_plain_text/fixed_width_no_space.txt)
...
* We can type `?read_fwf` into our R console to view the help documentation for this function and follow along with the explanation below.

* The first argument to the `read_fwf()` function is the `file` argument. The value passed to the file argument should be file path that tells R where to find the data set on our computer.
...
The final type of plain text file that we will discuss is by far the most common type used in our experience. I'm talking about the **comma separated values** or **csv** file. Unlike space and tab separated values files, csv file names end with the **.csv** file extension. Although, csv files are plain text files that can be opened in plain text editors such as Notepad for Windows or TextEdit for Mac, many people view csv files in spreadsheet applications like Microsoft Excel, Numbers for Mac, or Google Sheets. 
...
[Click here to download this file.](https://github.com/brad-cannell/r4epi/blob/master/chapters/importing_plain_text/comma.csv)
...
* We can type `?read_csv` into our R console to view the help documentation for this function and follow along with the explanation below.

* Like `read_tsv()`, R is basically executing  the `read_delim()` function with the value of the `delim` argument set to `","` under the hood. We could also use the `read_delim()` function with the value of the `delim` argument set to `","` if we wanted to.

## Additional arguments

For the most part, the data we imported in all of the examples above was relatively well behaved. What we mean by that is that the data basically "looked" like each of the `read_` functions were expecting it to "look". Therefore, we didn't have to adjust many of the various `read_` functions' default values. The exception was changing the default value of the `na` argument to the `read_delim()` function. However, all of the `read_` functions above have additional arguments that we may need to tweak on occasion. The two that we tend to adjust most often are the `col_names` and `col_types` arguments. It's impossible for us to think of every scenario where we may need to do this, but we'll walk through a basic example below, which should be sufficient to get the idea.

Take a look at this csv file for a few seconds. It started as the same exact height and weight data we've been using, but we made a few changes. See if you can spot them all.
...
When people record data in Microsoft Excel, they do all kinds of crazy things. In the screenshot above, I've included just a few examples of things we see all the time. For example:
...
::: callout-warning
* We suggest that you don't **EVER** alter raw data. All kinds of crazy things happen with data and data files. If we keep our raw data untouched and in a safe place, in a worst case scenario we can always come back to it and start over. If we start messing with the raw data, then we may lose the ability to recover what it looked like in its original form forever. If we import the data into R before altering it then our raw data stays preserved

* If the data need to be altered in Excel prior to import, we **strongly** suggest making a copy of the raw data first. Then, alter the copy before importing into R. But, even this can be a bad idea.

* When making alterations to the data in Excel then there is generally no record of those alterations. For example, let's say you click in a cell and delete a value (maybe even by accident), and then send someone the Excel file. They will have no way of knowing that a value was deleted. When we alter the data directly in Excel (or any program that doesn't require writing code), it can be really difficult for others (including future us) to know what was done to the data. We may be able manually compare the altered data to the original data if we have access to both, but who wants to do that -- especially if the file is large? However, if we import the data into R as-is and programmatically make alterations with R code, then our R code will, by definition, serve a record of all alterations that were made.

* Often data is updated. We could spend a significant amount of time altering our data in Excel only to be sent an updated file next week. Often, the manual alterations we made in one Excel file are not transferable to another. However, if all alterations are made in R, then we can often just run the exact same code again on the updated data.
:::
...
[Click here to download this file.](https://github.com/brad-cannell/r4epi/blob/master/chapters/importing_plain_text/comma_complex.csv)
...
* We can type `?readr::cols` into our R console to view the help documentation for this function and follow along with the explanation below.

* Notice various column types (e.g., `col_character()`) _are functions_, and that they are nested inside of the `cols()` function. Because they are functions, we must include the parentheses. That's just how the `readr` package is designed.
...
* We can type `?readr::cols` into our R console to see all available column types.
