...
In R, the most basic type of code script is simply called an R script. An R script is just a plane text document that contains R code and comments. R script files end with the file extension `.R`. 
...
1. They are simpler than R markdown documents, so they are a good place to start.   
2. Some of what I discuss below will also apply to R markdown documents.    
3. R scripts _are_ a better choice than R markdown documents in some situations (e.g., writing R packages, creating Shiny apps).   
...
To create your own R scripts, click on the icon shown below \@ref(fig:new-r-script1) you will get a dropdown box with a list of files you can create. \@ref(fig:new-r-script2)

```{r new-r-script1, echo=FALSE, fig.cap="Click the new source document icon."}
...
```{r new-r-script2, echo=FALSE, fig.cap="New source document options."}
