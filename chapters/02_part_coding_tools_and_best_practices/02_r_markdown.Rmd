...
It's actually kind of difficult for us to _describe_ what an Quarto file is if you've never seen or heard of one before. Therefore, we're going to start with an example and work backwards from there. Figure \@ref(fig:example-quarto) below is an Quarto file. It includes the exact same R code and comments as the example we saw in the chapter on creating R scripts. \@ref(fig:example-script)

# 🔴 Left off here... need to update the image below
