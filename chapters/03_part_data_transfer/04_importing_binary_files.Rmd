...
Technically, base R does not contain any functions that can be used to import the binary file types discussed above. However, the `foreign` package contains functions that may be used to import SAS data sets and Stata data sets, and is installed by default when you install R on your computer. Having said that, we aren't going to use the `foreign` package in this chapter. I'm going to use the following packages to import data in the examples below. If you haven't done so already, I suggest that you go ahead and install these packages now. 
...
[You may click here to download this file to your compter.](https://www.dropbox.com/s/sckq4owfttijnh5/excel.xlsx?dl=1)
...
[You may click here to download this file to your compter.](https://www.dropbox.com/s/ldhq5wd17687f1b/excel_complex.xlsx?dl=1)
...
As I said above, the `readr` package and `readxl` package were developed by the same people. So, the code above looks similar to the code we used to import the csv file in the previous chapter. Therefore, I'm not going to walk through this code step-by-step. Rather, I'm just going to highlight some of the slight differences.
...
In my experience, you are far more likely to have problems importing binary files saved from other statistical analysis applications. Perhaps because they are more complex, the data just seems to become corrupt and do other weird things far more often than is the case with plain text files. However, in my experience, it is also the case that when we are able to import binary files created in other statistical analysis applications, doing so requires less adjusting of default values. In fact, we will usually only need to pass the file path to the correct `read_` function.
...
[You may click here to download this file to your compter.](https://www.dropbox.com/s/zuu7h4qk4vdtcvt/height_and_weight.sas7bdat?dl=1)
...
* Because this is a large data frame (437,436 observations and 275 variables), we used the `head()` function to print only the first 10 rows of the data to the screen. 
...
* Because this is a large data frame (9,254 observations and 46 variables), we used the `head()` function to print only the first 10 rows of the data to the screen. 
...
[You may click here to download this file to your compter.](https://www.dropbox.com/s/2n6q3ntrd9y57al/height_and_weight.dta?dl=1)
...
You now know how to write code that will allow you to import data stored in all of the file formats that we will use in this book, and the vast majority of formats that you are likely to encounter in your real-world analyses. In the next section, I will introduce you to a tool in RStudio that makes importing data even easier. 
