...
We previously learned how to create an R data frame by directly creating data frames in RStudio with the `data.frame()` function, the `tibble()` function, or the `tribble()` function. This will get the job done, but it’s not always very practical, particularly when you have larger data sets. 
...
There are many different file types that one can use to store data. In this chapter we will focus on data stored in plain text files. You can think of plain text files as simple files that don't require any special software to view and edit. Your operating system's plain text editor is all you need (i.e., Notepad on Windows or TextEdit on Mac).

Later, we will discuss importing binary files. You can think of binary files as more complex file types that do require special software to view and edit. For example, Microsoft Excel spreadsheets, SAS data sets, and Stata data sets.
...
Base R contains several functions that can be used to import all of the file types discussed above; however, I'm going to use the following packages to import data in the examples below. I suggest that you go ahead and install these packages now if you would like to follow along. 

* [readr](https://readr.tidyverse.org/). We will use the `readr` package to import plain text and csv files.
...
In this chapter, we will be using the `readr` package extensively. I will go ahead and load it now.
...
* The `readr` package imported the data and printed a message giving us some information about how it interpreted column names and column types. In programming lingo, deciding how to interpret the data that is being imported is called **parsing** the data.
  ...
  - By default, `readr` will assume that the first row of data will contains variable names and will try to use them as column names in the data frame it creates. In this case, that was a good assumption. We want the columns to be named id, sex, ht_in, and wgt_lbs. Later, we will learn how to override this default behavior.
  
  - By default, `readr` will try to guess what type of data (e.g., numbers, character strings, dates, etc.) each column contains. It will guess based on analyzing the contents of the first 1,000 rows of the data. In this case, the `readr`'s guess were not correct. The variables id and sex should be character variables, but ht_in should not be. Below, we will learn how to fix this issue.
  
<p class="warning"> ⚠️**Warning:** Make sure to always include the file extension in your file paths. For example, using "/Users/.../single_delimited" instead of "/Users/.../single_delimited.txt" above (i.e., no .txt) would have resulted in an error telling you that the filed does not exist.</p>
...
Did you see the period in the third value of the second column? The period is there because this value is missing, and a period is commonly used to represent missing data. However, R represents missing data with the special `NA` value -- not a period. So, the period is just a regular character value to R. When R reads the values in the `ht_in` column, it decides that it can easily turn the numbers into character values, but it doesn't know how to turn the period into a number. So, the column is parsed as a character vector. 
...
* We told R to look for a period to represent missing data instead of a nothing or a single space by passing the period character to the `na` argument.
...
* The second argument to the `read_fwf()` function is the the `col_positions` argument. The value passed tells R the width (i.e., number of columns) that belong to each variable in the raw data set. This information is actually passed to the `col_positions` argument directly from the `fwf_widths()` function. This is an example of nesting functions.
...
```
  ...
  - All of the values for the variable `id` can be located within the first 3 columns of data. All of the values for the variable `sex` can be located within the next 6 columns of data. All of the values for the variable `ht_in` can be located within the next 5 columns of data. And, all of the values for the variable `wgt_lbs` can be located within the next 3 columns of data. Therefore, we case the vector `c(3, 6, 5, 3)` to the `widths` argument.
...
* The third argument of the `read_fwf()` function that we passed a value to is the `skip` argument. The value passed to the `skip` argument tells R how many rows to ignore before looking for data values. In this case, we passed a value of one, which told R to ignore the first row of the raw data. We did this because the first row of the raw data contained variable names instead of data values, and we already gave R variable names in the `col_names` argument to the `fwf_widths()` function.
...
The final type of plain text file that we will discuss is by far the most common type used in my experience. I'm talking about the **comma separated values** or **csv** file. Unlike space and tab separated values files, csv file names end with the **.csv** file extension. However, they are still considered plain text files. Meaning, you can open them in plain text editors such as Notepad for Windows or TextEdit for Mac. However, you can also open csv files in spreadsheet applications like Microsoft Excel, Numbers for Mac, and Google Sheets. 
...
* Row one contains generic variable names that don't really serve much of purpose. 

* Row one also contains some notes for the data collection staff that aren't really part of the data. 

* Row two is a blank line. I'm not sure why it's there. Maybe the study staff finds it aesthetically pleasing?

* Row three are some variable descriptions. These are actually useful, but they aren't currently formatted in a way that makes for good variable names.

* Row 7, column D is a missing value. However, someone wrote the word "Missing" instead of leaving the cell blank.

All of the issues listed above are things we will have to deal with before we can analyze our data. Now, in this small data set we could just fix these issues directly in Microsoft Excel and then import into R with a simple call to `read_csv()` without adjusting any options. However, that this is generally a really bad idea. 

<div class="warning"> ⚠️**Warning:** Alter your data with code 
...
* If you are going to make alterations in Excel prior to importing the data, I **strongly** suggest making a copy of the Excel file, and then altering and importing the copy of the raw data. But, even this can be a bad idea.

* If you make alterations to the data in Excel then there is generally no record of those alterations. Therefore, it can be really difficult for others (including future you) to know what was done to the data. You can manually compare the altered data to the original data of you are working from a copy (see above), but who wants to do that -- especially if the file is large? If you import the data into R as-is then your R code will, by definition, create a record of any alterations that were made. 

* Often data is updated. You could spend a significant amount of time altering your data in Excel only to be sent an updated file next week. Often, the manual alterations you made in one Excel file are not transferable to another. However, if all alterations are made in R then you can often run the exact same code again on the updated data. </div>

So, let's walk through addressing these issues together. We'll start by taking a look at our results with all of `read_csv`'s arguments left are their default values.
...
That is obviously not what we wanted. So, let's start adjusting some of the default values.
...
* We passed a character vector of variable names to the `col_names` argument. Doing so told R to use the words in the character vector as column names instead of the values in the first row of the raw data.
...
Wow! This was kind of a long chapter! 🤯 But, you should not have the foundation you need to start importing data in R instead of creating data frames manually. At least as it pertains to data that is stored in plain text files. Specially, you should now know how to import data stored in the following formats:
...
Next, we will learn how to import data that is stored in binary files. Most of the concepts we learned in this chapter will apply, but we will get to use a couple new packages 📦.
