...
Okay, celebration complete. Now, what does he do with this new GitHub repository? Well, he do the four things covered in [Introduction to git and GitHub].
...
1. Click `GitHub.com` in the clone menu. This tells GitKraken that the repository he wants to clone currently lives on his GitHub account. Note that it has to be on **his** account -- not someone else's account. We will learn how to get files from from someone else's account later.   
...
This step is technically optional, but we highly recommend it! We introduced [R projects] earlier in the book. Arthur will go ahead and add an R project file to his repository now. This will make his life easier later. To create a new R project, he just need to click the drop-down arrow next to the words `Project: (None)` to open the projects menu. Then, he will click the `New Project...` option.
...
Arthur can continue adding R code and other files to his local repository and pushing them to GitHub in this fashion for the remainder of the time he is working on this project, and the [introduction to git and GitHub chapter][Introduction to git and GitHub] discusses _why_ he should consider doing so.
...
We will answer questions 1 & 2 now. We will answer the third question in the [next workflow example][Example 3: Contribute to a research project].
...
When our research assistants begin helping us with data management and analysis projects, we often have them start by going to the project’s GitHub repository to read the existing documentation and [clone][Step 2: Clone the repository to your computer] all the existing code to their computer. This example is going to walk us through that process step-by-step. For demonstration purposes, we will work with the example repository that our fictitious research assistant named Arthur Epi created in [Example 2 above][Example 2: Create a repository for a research project]. If you want to follow along, you can use your GitHub account in place of Arthur’s.
...
As we've pointed out multiple times in this chapter, we generally do not want to upload research data to GitHub. Why? Because it isn't typically considered private or secure. 
...
How to share the data if not via GitHub?
