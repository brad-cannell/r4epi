...
There will often be times that we want to modify the values in our data in some way based on the values in one or more columns in our data. For example, maybe we want to create a column that contains the region of the country someone is from, based on another column that contains the state they are from. 
...
Just like I have to ask my daughter to put on a raincoat using conditional logic, I have to ask sometimes ask R to execute commands using conditional logic, and I have to put them in a form that R understands. One such form is `dplyr`'s `if_else()` function. Let's go ahead and take a look at an example now:
...
Now, let's say that we want to create a new column in our data frame called `raincoat`. We want the value of `raincoat` to be `wear` rainy days and `no wear` on days when it isn't raining. Here's how we can do that with the if_else() function:
...
* The second argument to the `if_else()` function is the `true` argument. The value passed to the `true` argument tells R what value the `if_else()` function should return when the condition is `TRUE`. In this case, we told R to return the character value `wear`.

* The third argument to the `if_else()` function is the `false` argument. The value passed to the `false` argument tells R what value the `if_else()` function should return when the condition is `FALSE`. In this case, we told R to return the character value `no wear`.
...
* The value we passed to the `condition` argument was `NA`. In doing so, we asked R to check each value of the `name_match` and see if it was `NA`. 

* If it was `NA`, then we want to return the value that we passed to the `true` argument. Somewhat confusingly, the value we passed to the `true` argument was `FALSE`.  

* If the value in `name_match` was NOT `NA`, then we want to return the value that we passed to the `false` argument. In this case, we asked R to return the value that already exists in the `name_match` column.
...
* Participant 2 had a systolic blood pressure of 120 and a diastolic blood pressure of 60. Although 60 is less than 80 (condition number 2), 120 is not less than 120 (condition number 2). So, the value returned by the `if_else()` function was `Not Normal`.
...
* The `case_when()` function doesn't have a direct equivalent to the `if_else()` functions `false` argument. Instead, it evaluates each two-sided formula sequentially, until if finds a condition that is met. If it never finds a condition that is met, then it returns an `NA`. I will expand on this more below.
...
The `case_when()` function was really useful for creating the `bp` column above, but there was also a lot going on there. Let's take a look at a slightly less complex example and clarify a few things along the way. 

## Recoding variables

In epidemiology, recoding variables is really common. For example, we may collect information about people's ages as a continuous variable, but decide to that it makes more sense to **collapse** age into age categories for our analysis. Let's say that our analysis plan calls for assigning each of our participants to one of the following age categories: 
...
* Notice that I named the new variable `age_3cat`. I'm not sure where I picked up this naming convention, but I use it a lot when I collapse variables. The basic format is name of variable I'm collapsing, an underscore, and the number of categories in the collapsed variable. I like using this convention for two reasons. First, it's meaningful and informative. Second, I don't have to spend any time trying to think of a different meaningful or informative name for my new variable. It's totally fine if you don't adopt this naming convention, but I would recommend that try to use names that are more informative than `age2` or something like that.
...
* Notice that this time we passed two two-sided formulas to the `case_when()` function -- `age < 12 ~ 1` and `age >= 12 & age < 18 ~ 2`. 
  ...
  - Above, the first value in `age` is `15`. `15` is _NOT_ less than `12`. So, `case_when()` moves on to evaluate the next two-sided formula. `15` is greater than or equal to `12` _AND_ `15` is less than `18`. Because both conditions of the second two-sided formula were met, `case-when()` returns the value on the right hands side of the second two-sided formula -- `2`. So, the first value returned by the `case_when()` function is `2`. 
  
  - The second value in `age` is `19`. `19` is _NOT_ less than `12`. So, `case_when()` moves on to evaluate the next two-sided formula. `19` is greater than or equal to `12`, but `19` is _NOT_ less than `18`. So, `case_when()` tries to move on to the next two-sided formula. However, there is no next two-sided formula. So, the first value returned by the `case_when()` function is `NA`.

  -  The fourth value in `age` is `3`. `3` is less than `12`. So, the fourth value returned by the `case_when()` function is `1`. **At this point, `case_when()` does not continue checking two-sided formulas. It moves on to the next value of `age`.**
...
* In everyday speech, we may express the second two-sided condition above as "categorize all people between the ages of 12 and 18 as an adolescent." I want to make two points about that before moving on.
  ...
  - First, while that statement may be totally reasonable in everyday speech, it is quite specific enough for what we are trying to do here. Between 12 and 18 is a little bit ambiguous. What category is a person put in if they are exactly 12? What category are they put in if they are exactly 18? So, clearly we need to be more precise. I'm not aware of any hard and fast rules, but I tend to _include_ the lower end of the range in the current category and _exclude_ the value on the upper end of the range in the current category. So, in the example above, I would say, "categorize all people between the ages of 12 and less than 18 as an adolescent."
  
  - Second, when we are testing for a "between" condition like this one, I often see students write code like this: `age >= 12 & < 18`. R won't understand that. You have to use the column name in each condition to be tested, even doesn't change. Otherwise, you get an error that looks something like this:
...
Next, let's wrap up this age category variable:
...
We've already talked about how R uses the special `NA` value to represent missing data. We've also learned how to convert other representation of missing data (e.g., ".") to `NA` when we are importing data. However, It is extremely common for data sets that we use in epidemiology to include "don't know" and "refused" answer options. By convention, those options are often coded as `7` and `9`. For questions that include 7 or more response options (e.g., month), then `77` and `99` are commonly used to represent "don't know" and "refused". For questions that include 77 or more response options (e.g., age), then `777` and `999` are commonly used to represent "don't know" and "refused". 
  
While differentiating between true missing (i.e., the respondent was never asked the question or just left the response blank on a written questionnaire), don't know (i.e., the respondent doesn't know the answer), and refused to answer (i.e., the respondent knows the answer, but doesn't want to reveille it -- possibly out of shame, fear, or embarrassment) can be of some interest for survey design purposes, but they typically all amount to missing data by the time you get around to the substantive analyses. In other words, knowing that a person refused to give their age doesn't help me figure out how old they are any more than if they had never been asked at all. Therefore, we commonly use conditional operations in epidemiology to recode these kinds of values to missing. 
...
The code above works, and it is very explicit. However, we can definitely make it more succinct and easier to read.
