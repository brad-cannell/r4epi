...
Up to this point, the data we’ve needed has always been stored in a single data set or data frame. However, that won’t always be the case. 

At times you may need to combine data from multiple agencies in order to complete your analysis. 
...
Or, data is sometimes gathered over long periods of time. When this happens, it is not uncommon for observations for the sites or times to be stored as separate data sets.
...
This combining of data comes in two basic forms: combining vertically and combining horizontally. First you’ll learn about combining vertically, or adding rows. Later, you’ll learn about combining horizontally, or adding columns.
...
Now you want to combine these two data frames and end up with one data frame that includes the observations from data frame two listed directly below the observations from data frame one. In this chapter you will learn how combine these data frames vertically.
...
Suppose you are working on a multisite clinical trial recruiting participants from Chicago, San Diego, Denver, and Fort Worth over multiple years. You have a data frame named Trial, that stores the number of participants recruited each year, as well as the number of participants who experienced the outcome of interest. Another data frame named Trial_2020 was just sent to you with the recruitment numbers for the year 2020. 
...
What happens when the data frames we want to combine have identical sets of columns? For example, let's say that we started collecting data on adverse events for the first time in 2020. In this case, `trials_2020` would contain a column that `trials` doesn't contain. Could we still row bind our two data frames? Let's see:
...
Our goal is once again to combine these data frames. But, this time we want to combine them horizontally. In other words, we want a combined data frame that contains all the columns in data frame one _and_ data frame two.
...
* We used `dplyr`'s `bind_cols()` function to horizontally bind the columns in `demographics` to the columns in `grip_strength`.
...
Suppose we are analyzing data from a study of aging and functional ability. At baseline, we assign a study id to each of our participants. We then ask them their date of birth and their race and ethnicity. We saved that information in a data file called `demographics`. 
...
Then, we asked our participants to do a series of functional tests. The functional tests include measuring grip strength in their right hand (`grip_r`) and grip strength in their left hand (`grip_l`). We save each measure, along with their study id, in a separate file called `grip_strength`.
...
* Notice the message that `bind_cols()` gave us this time: `New names: * id -> id...1 * id -> id...2`. This is telling us that both data frames had a column named `id`. If we `bind_cols()` had left the column names as-is, then the resulting combined data frame would have had two columns named `id`, which isn't allowed.
...
* Notice that the result above only includes a single `id` column. This is because we aren't simply smooshing two data frames together, side-by-side. We are integrating data from across the two data frames based on the value of the key column -- `id`.
...
Now, to conduct our analysis, we need to combine the data in `demographics` with the data in the longitudinal `grip_strength` data frame. And how will we ask R to merge these two data frames? Well, here is some good news. To perform a one-to-many or many-to-many merge, we use the exact same syntax as you used to perform a one-to-one merge. R will figure out the relationship between the data frames automatically. Take a look:
...
Let's throw one more little wrinkle into our analysis. Let's say that each participant had a medical exam prior to being sent into the gym to do their functional assessments. The results of that medical exam, along with the participant's study id, we recorded in the university hospital system's electronic medical records. As part of that medical exam, each participant's weight was recorded. Luckily, we were given access to the electronic medical records, which look like this:
...
Of course, that doesn't work because `left_join()` can only merge two data frames at a time -- `x` and `y`. So, our next attempt might look like this:
...
But, if you look closely, that isn't what we want either. Each participant didn't have four visits. They only had two. The problem here is that our second merge has two key columns now instead of one. In other words, we need to align every row in the combined `demographics`/`grip_strength` data with its matching row in the `emr` data by `id` _and_ `visit`. Here is how we do that:
...
* We used `dplyr`'s `left_join()` function to perform a one-to-many merge of the `demographics` data frame with the `grip_strength` data frame. Then, we used `left_join()` again to perform a one-to-many merge of the combined `demographics`/`grip_strength` data frame with the `emr` data frame.

* We told `left_join()` that it needed to match the values in the `id` key column _and_ the values in the `visit` key column in order to align the rows in the combined `demographics`/`grip_strength` data frame with the `emr` data frame. In other words, it wasn't enough to ask `left_join()` to "match id 1 with id 1." Instead we had to ask `left_join()` to "match id 1, visit pre with id 1, visit pre." We did so by passing a character vector of key column names to the `by` argument.
